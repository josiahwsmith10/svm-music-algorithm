...
function [R,M,antLoc,lambda,X] = generateDoASignalNonuniform(doas,nAnt,nSymbols,lenCarrier,noisePower)
...
antLoc = dist*[0:round(L/2),round(L/2)+1:2:(round(L/2)+1+2*(L-1-(round(L/2)+1)))].';% Distances among antennas
...
% figure; scatter(antLoc/lambda,zeros(size(antLoc)),'x');xlabel("\lambda");title("Linear Nonuniform Array")
